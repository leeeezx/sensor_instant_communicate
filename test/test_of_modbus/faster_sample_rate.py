...
class SensorReader:
    def __init__(self, port, slave_address, baudrate=9600, interval=0.001):
        """
        初始化传感器读取器的相关参数
    ...
    def read_float(self, register_address, precision_bit=2):
        ...
        if current_time - self.last_read_time < self.interval:
            time.sleep(self.interval - (current_time - self.last_read_time))
...
def main():
    ...
    INTERVAL = float(input("请输入读取间隔时间（秒，默认0.01）: ") or "0.01")
    ...
    sensor = SensorReader(PORT, SLAVE_ADDRESS, BAUDRATE, INTERVAL)
    print("\n开始读取数据，按 Ctrl+C 停止...")
    sample_count = 0
